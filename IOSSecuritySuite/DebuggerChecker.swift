...
internal class DebuggerChecker {
    ...
    static func amIDebugged() -> Bool {
        ...
        if sysctlRet != 0 {
            print("Error occured while calling sysctl(). The debugger check may not be reliable")
        }
    ...
    static func denyDebugger() {
        ...
        if ptraceRet != 0 {
            print("Error occured while calling ptrace(). Denying debugger may not be reliable")
        }
    }
